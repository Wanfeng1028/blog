...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!allowedTypes.has(file.type)) return apiError("Unsupported image type", 415);
    if (file.size > getLimitInBytes()) return apiError("File is too large", 413);

    const key = `avatar-${Date.now()}-${randomUUID()}`;
