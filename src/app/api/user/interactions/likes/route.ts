...
import { getUserLikes } from "@/features/user/server/dashboard";
...
export async function GET() {
  ...
  const items = await getUserLikes(session.user.id);
  return apiOk(
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return apiError(error instanceof Error ? error.message : "Failed to remove like", 500);
  }
}
